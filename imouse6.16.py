...
class _IMouseProClient:
    ...
    def _post(self, fun, data, quiet=False):
        try:
            r = self.session.post(self.base_url, json={"fun":fun,"data":data,"msgid":0}, timeout=30)
            return r.json()
        except: return None
    def get_devices(self):
        r = self._post("get_device_list", {})
        if r and r.get("status")==0:
            d = r.get("data",{})
            return [v for v in d.values() if isinstance(v,dict)] if isinstance(d,dict) else (d if isinstance(d,list) else [])
        return []
    ...
    def open_url(self, did, url):
        return self._post("shortcut",{"deviceid":did,"id":13,"devlist":[did],"parameter":json.dumps({"url":url}),"outtime":15000})
    def get_clipboard(self, did):
        r = self._post("shortcut",{"deviceid":did,"id":11,"outtime":15000}, quiet=True)
        if r is None or r.get("status")!=0: return None
        rd = r.get("retdata",{})
    ...
    def find_image(self, did, img_b64, similarity=0.8, rect=None):
        ...
        data = {"deviceid":did,"img":img_b64,"similarity":similarity}
        if rect: data["rect"]=rect
        r = self._post("find_image", data, quiet=True)
        if r and r.get("status")==0:
            rd=r.get("data",{}); result=rd.get("result"); conf=rd.get("confidence",0)
            if result and len(result)>=2: return (result[0],result[1],conf)
        return None
    def ocr(self, did, rect=None):
        """OCR文字识别"""
        data = {"deviceid":did,"original":False}
        if rect: data["rect"]=rect
    ...
    def drag(self, did, sx, sy, ex, ey, duration=500, steps=20):
        ...
        self._post("mouse_move", {"deviceid": did, "x": sx, "y": sy})
        time.sleep(0.1)
        self._post("mouse_down", {"deviceid": did, "button": "left"})
        time.sleep(1.0)
        for i in range(1, steps + 1):
            mx = int(sx + (i / steps) * (ex - sx))
            my = int(sy + (i / steps) * (ey - sy))
            self._post("mouse_move", {"deviceid": did, "x": mx, "y": my})
            time.sleep(0.05)
        time.sleep(0.1)
        self._post("mouse_up", {"deviceid": did, "button": "left"})
    ...
    def fix_landscape(self, did, log=None):
        ...
        try:
            ...
            ss = self._post("get_device_screenshot", {"deviceid": did, "isJpg": True, "gzip": False, "original": False})
            if not ss or ss.get("status") != 0:
                return False
            _d = ss.get("data", {})
...
class _TikHubClient:
    ...
    def fetch_user_videos(self, sec_uid, max_count=30, publish_after=None, unique_id=None):
        ...
        返回视频列表（可能为空）。只有所有接口都失败才走 fallback；
        某个接口成功返回数据（即使被 publish_after 过滤为空）就直接返回，不再走后面 fallback。
        """
        # 1) V3 (最快)
        videos, api_ok = self._fetch_videos_appv3(sec_uid, max_count, publish_after, ep="fetch_user_post_videos_v3", unique_id=unique_id)
        if api_ok: return videos
        # 2) V2
        videos, api_ok = self._fetch_videos_appv3(sec_uid, max_count, publish_after, ep="fetch_user_post_videos_v2", unique_id=unique_id)
        if api_ok: return videos
        # 3) V1
        videos, api_ok = self._fetch_videos_appv3(sec_uid, max_count, publish_after, ep="fetch_user_post_videos", unique_id=unique_id)
        if api_ok: return videos
        # 4) Web (需要 sec_uid)
    ...
    def _fetch_videos_appv3(self, sec_uid, max_count, publish_after, ep="fetch_user_post_videos_v3", unique_id=None):
        ...
        返回 (videos, api_ok): api_ok=True 表示接口正常返回（即使数据被过滤后为空），False 表示接口失败
        """
        videos=[]; cursor=0; api_ok = False
        while len(videos)<max_count:
            ...
            if not d: return [], False  # API失败，让上层 fallback
            inner=d.get("data",{})
            aweme_list = inner.get("aweme_list") or []
            api_ok = True  # 接口正常返回了（即使列表为空也算成功）
            if not aweme_list and cursor == 0: return [], True  # 该账号确实没视频
            for a in aweme_list:
...
def _get_screen_b64(c, did):
    ...
    # 再尝试 Pro 风格
    try:
        ss = c._post("get_device_screenshot", {"deviceid": did, "isJpg": True, "gzip": False, "original": False})
...
# ── 投流码: 完整流程入口 ──
def run_adcode_full(phase="all", port=9912, workers=20, days=1, videos_str="", match_mode="name", log_fn=print):
    ...
    if phase in ("all","2"):
        ...
        ic=_IMouseProClient("127.0.0.1",port)
        ...
        def _run_one(idx,item):
            ...
            c2=_IMouseProClient("127.0.0.1",port)
...
class MyApp(QtWidgets.QMainWindow):
    ...
    def _fetch_video_data_thread(self):
        ...
        try:
            run_adcode_full(
                phase="1", port=IMOUSE_PRO_PORT, workers=PHASE2_DEFAULT_WORKERS,
    ...
    def _adcode_selected_thread(self, dates, online_devices, match_mode):
        ...
        try:
            ...
            for did, info in online_devices.items():
                key = (info.get('name', '') if match_mode == 'name' else info.get('username', '')).strip()
                if key:
                    dev_map[key] = did
    ...
    def _adcode_thread(self, match_mode):
        ...
        try:
            result = run_adcode_full(
                phase="2", port=IMOUSE_PRO_PORT, workers=PHASE2_DEFAULT_WORKERS,
                    ...
                    try:
                        ...
                        def _grab_rate(_c, _did, _by, _name, _log):
                            ...
                            try:
                                ...
                                ss = _c._post("get_device_screenshot",
                                              {"deviceid": _did, "isJpg": True, "gzip": False, "original": False})
                                if not ss or ss.get("status") != 0:
    ...
    def _switch_single(self, did, name):
        """单台设备切号流程（Pro接口版）"""
        c = _IMouseProClient(self.__server, self.__port)
        ...
        for attempt in range(3):
            ...
            try:
                ...
                r = c._post("get_device_screenshot", {"deviceid": did, "isJpg": True, "gzip": False, "original": False}, quiet=True)
                img_data = None
                if r and r.get("status") == 0:
            ...
            row_ys = []
            try:
                r2 = c._post("get_device_screenshot", {"deviceid": did, "isJpg": True, "gzip": False, "original": False}, quiet=True)
                img_data2 = None
